...
def explain():
    ...
    Explication globale : feature_importance du modèle
    """
    try:
    ...
    try:
        ...
        if hasattr(model, "booster_"):           
            ...
            return jsonify({
                "error": "Explication indisponible: modèle non compatible pred_contrib."
            }), 400

        # --- 1. EXPLICATION LOCALE ---
        ...
        # --- 2. EXPLICATION GLOBALE ---
        global_imp = []
        imp_vals = None
        if hasattr(booster, "feature_importance"):
            imp_vals = booster.feature_importance(importance_type='split')
        ...
        if imp_vals is not None:
            ...
            if total_imp > 0:
                imp_vals = [x / total_imp for x in imp_vals] # Normalisation en %
            
            for feat, imp in zip(expected_cols, imp_vals):
                global_imp.append({
                    "feature": feat,
                    "importance": float(imp)
                })
            
            # Trier l'importance globale de la plus grande à la plus petite
        ...
        return jsonify({
            ...
            "threshold": float(thr),
            "top": top,
            "global_importance": global_imp[:20] # Top 20 pour la perf
        })
